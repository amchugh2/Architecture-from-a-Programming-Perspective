...
add_key_value(struct KeyValue *keyValues, const char *k, int v)
{
  ...
  // struct KeyValue *: pointer that points to KeyValue struct (not neccesarily memory location)
  // *: 8 bytes (size of pointer)
 struct KeyValue* kv =  malloc(sizeof(struct KeyValue));

  //allocate storage for string pointed to by k
  char* s = malloc(strlen(k)+ 1); // account for terminating character
...
free_key_values(struct KeyValue *keyValues)
{
  ...
  //fix this loop: using p-> succ
  struct KeyValue* p = keyValues;
  while( p != NULL){
    // free takes a VOID Not a const void* must change
    // free key (cast)
